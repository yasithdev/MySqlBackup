...
namespace MySqlBackupTestApp
{
    ...
    {
        private readonly string ConnectionSettingFile = Path.Combine(Environment.CurrentDirectory,
            "ConnectionSettings.txt");

        private readonly List<Form> lstForm = new List<Form>();
        private bool stopWrite = true;

        public FormMain()
        {
            InitializeComponent();
            Text += ", Loaded MySqlBackup.DLL Version: " + MySqlBackup.Version;
            LoadSettings();
        }
        ...
        private void button_ExportAs_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == f.ShowDialog())
            {
                ...
                Program.DefaultFolder = Path.GetDirectoryName(textBox_File.Text);
            }
        }
        ...
        private void LoadSettings()
        {
            try
            {
                if (File.Exists(ConnectionSettingFile))
                {
                    textBox_Connection.Text = File.ReadAllText(ConnectionSettingFile);
        ...
        private bool WriteSettings(bool forceSave)
        {
            try
            {
                ...
                if (forceSave)
                {
                    File.WriteAllText(ConnectionSettingFile, textBox_Connection.Text);
                }
                else
                {
                    if (File.Exists(ConnectionSettingFile))
        ...
        private void cbAutosave_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (cbAutosave.Checked)
            {
                if (WriteSettings(true))
                    MessageBox.Show(
                        "Automatic save enabled." + Environment.NewLine + "Connection String saved at" +
                        Environment.NewLine + Environment.NewLine + ConnectionSettingFile, "Saving Connection String");
            }
            else
            {
                try
                {
                    File.Delete(ConnectionSettingFile);
                }
                catch
                {
                }
